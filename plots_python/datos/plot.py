...
En, cs = np.genfromtxt("cs_eE_Zh.dat",dtype=float,unpack=True,skip_header=True) 
En2, cs2 = np.genfromtxt("cs_4F_HZ.dat",dtype=float,unpack=True,skip_header=True) 
...
plt.ylabel("$\\sigma(e^+e^-\\rightarrow Xh)$ (fb)", fontsize=20)
plt.xlim(200,1000)
#plt.ylim(0.01,10000)
#plt.yscale('log')

plt.plot(Ener, SIG_1, linestyle='-',color='r',linewidth=2, label='$Zh$ CalcHEP')
plt.plot(Ener, SIG_2, linestyle='-',color='b',linewidth=2, label='$WW$ Madgra')
#plt.plot(Ener, SUM_SIG, linestyle='-',color='g',linewidth=2, label='both')
